...
public class ThymeleafGenerator extends AbstractThymeleafGenerate {
	...
	public ThymeleafGenerator(String className, String parameters) throws IOException {
		...
		generateViewHtml(className, parameters);
	}
	...
	public void generateIndexHtml(String className, String parameters) throws IOException {
		...
		String template = "~{layout.html}"; //"layout";
	...
	public void generateViewHtml(String className, String parameters) throws IOException {
		String htmlString =IOUtils.toString(getClass().getResourceAsStream("/templates/template-view.html"), null);

		String template = "~{layout.html}"; //"layout";
		String paramClassName = className.toLowerCase();
		String pathUrl = "/" + className.toLowerCase() + "s";
		String viewAttributes = generateViewParameters(paramClassName, parameters);
		
		htmlString = htmlString.replace("${template}", template);
		htmlString = htmlString.replace("${className}", className);
		htmlString = htmlString.replace("paramClassName", paramClassName);
		htmlString = htmlString.replace("/path_url", pathUrl);
		htmlString = htmlString.replace("${viewAttributes}", viewAttributes);
		
		File newHtmlFile = new File(getUserDir() + "/src/main/resources/templates/" + className.toLowerCase() + "/view.html");
		
		FileUtils.writeStringToFile(newHtmlFile, htmlString);
		System.out.println("create /src/main/resources/templates/" + className.toLowerCase() + "/view.html");
	}
